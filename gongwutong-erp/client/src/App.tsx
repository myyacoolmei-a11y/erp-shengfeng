...
import { AuthProvider, useAuth, hasRole, userHasFeature, userHasModule, defaultPathForRole, type UserRole } from "@/contexts/auth-context";
...
function FeatureGuard({
  ...
  const required = modules ?? (module ? [module] : []);
  if (required.length > 0 && !required.some((m) => userHasModule(user, m))) {
    return <AccessDenied />;
  }
  return <>{children}</>;
}
...
function Guard({
  ...
  const inner = (
    <FeatureGuard feature={feature} module={module} modules={modules}>
      {children}
    </FeatureGuard>
  );
...
function EngineerOrFeatureGuard({
  ...
  const [location] = useLocation();
  if (!user) return <Redirect to="/login" />;
  if (feature === "dispatch_orders" && !userHasModule(user, "dispatch")) {
    const fallback = defaultPathForRole(user);
...
function OperationCenterRoute() {
  const { user } = useAuth();
  if (!userHasModule(user, "subsidy")) return <AccessDenied />;
  return <OperationCenterPage />;
}
      ...
      <Route>
        <Layout>
          <Switch>
            ...
            <Route path="/payments">
              <Guard feature="receivables" module="payments"><Payments /></Guard>
            </Route>
            <Route path="/receivables">
              <Guard feature="receivables" modules={["accounts_receivable", "invoices"]}><Receivables /></Guard>
            </Route>
