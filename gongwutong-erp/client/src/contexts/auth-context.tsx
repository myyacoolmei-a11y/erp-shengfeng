...
export function userCanAccessNav(user: AuthUser | null, href: string): boolean {
  if (!user) return false;
  if (!navHrefAllowed(user, href)) return false;
  const feature = featureForPath(href);
  return companyAllowsPath(user.companyModules ?? [], href, feature);
}

export function userHasFeature(user: AuthUser | null, feature: FeatureKey): boolean {
  if (!user) return false;
  if (!hasFeaturePermission(user, feature)) return false;
  return companyAllowsFeature(user.companyModules ?? [], feature);
}

export function userHasModule(user: AuthUser | null, module: ModuleKey): boolean {
  if (!user) return false;
  return isModuleEnabled(user.companyModules ?? [], module);
}
