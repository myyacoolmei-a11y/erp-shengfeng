...
import { useAuth, effectiveRoles, userCanAccessNav, userHasFeature, userHasModule, type UserRole, type AuthUser } from "@/contexts/auth-context";
...
function NavContent() {
  ...
  const showOperationCenter =
    ...
    userHasModule(user, "subsidy");

  const showModuleSettings = userRoles.includes("super_admin") || userRoles.includes("owner");
        ...
        <nav className="flex flex-col gap-0.5">
          ...
          {showModuleSettings && (
            <Link
              ...
              className={`${NAV_ITEM_BASE} ${
                location.startsWith("/company/modules") ? NAV_ITEM_ACTIVE : NAV_ITEM_INACTIVE
              }`}
            >
              <SlidersHorizontal
                className={`h-4 w-4 shrink-0 ${location.startsWith("/company/modules") ? "text-background" : "text-muted-foreground/70"}`}
                strokeWidth={1.5}
              />
              <span className="truncate">功能模組</span>
            </Link>
          )}
