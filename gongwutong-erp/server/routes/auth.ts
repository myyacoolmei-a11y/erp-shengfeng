...
import { toJwtUserFields, toAuthClientUser } from "../lib/userPublicDto";
import { logger } from "../lib/logger";
import { getDefaultCompany } from "../lib/companyModuleService";
...
router.post("/auth/login", async (req, res): Promise<void> => {
  ...
  const publicUser = await toAuthClientUser(user);
  res.json({
    token,
    user: publicUser,
  });
...
router.patch("/auth/password", authenticate, async (req, res): Promise<void> => {
  ...
  const publicUser = await toAuthClientUser({ ...user, mustChangePassword: false });
  res.json({
    token: newToken,
    user: publicUser,
  });
