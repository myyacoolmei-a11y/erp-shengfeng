...
import { requireRole, requireCompanyModule } from "../lib/auth";
import {
...
router.use("/operation-center", requireCompanyModule("subsidy"));
router.use((req, res, next) => {
  if (String(req.path).includes("subsidy")) {
    requireCompanyModule("subsidy")(req, res, next);
    return;
  }
  next();
});
