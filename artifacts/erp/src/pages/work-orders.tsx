...
import { useListWorkOrders, useCreateWorkOrder, useUpdateWorkOrder, useDeleteWorkOrder, useListCustomers, useListProgress, useCreateProgress, getListWorkOrdersQueryKey, getListProgressQueryKey } from "@workspace/api-client-react";
...
function ProgressPanel({ workOrderId }: { workOrderId: number }) {
  const queryClient = useQueryClient();
  const { data: progress } = useListProgress(workOrderId);
  const createMutation = useCreateProgress({ mutation: { onSuccess: () => { queryClient.invalidateQueries({ queryKey: getListProgressQueryKey(workOrderId) }); setNote(""); } } });
  const [note, setNote] = useState("");
  return (
    <div className="space-y-3 mt-3">
      <div className="space-y-1">
        {progress && progress.length > 0 ? progress.map(p => (
          <div key={p.id} className="text-xs border-l-2 border-primary/30 pl-2 py-1">
            <p className="font-medium">{p.description}</p>
            <p className="text-muted-foreground">{new Date(p.createdAt).toLocaleString("zh-TW")} {p.recordedBy && `· ${p.recordedBy}`}</p>
          </div>
        )) : <p className="text-xs text-muted-foreground">尚無進度紀錄</p>}
      </div>
      <div className="flex gap-2">
        <Input className="text-xs h-8" placeholder="新增進度紀錄..." value={note} onChange={e => setNote(e.target.value)} />
        <Button size="sm" className="h-8 text-xs" disabled={!note || createMutation.isPending} onClick={() => createMutation.mutate({ workOrderId, data: { description: note } })}>新增</Button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
              ...
              <div key={o.id} className="px-4 py-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1 ml-2">
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setExpandedId(expandedId === o.id ? null : o.id)}><Eye className="h-3.5 w-3.5" /></Button>
                ...
                {expandedId === o.id && <ProgressPanel workOrderId={o.id} />}
              </div>
            ))}
          </div>
        </CardContent></Card>
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={e => { e.preventDefault(); mode === "create" ? createMutation.mutate({ data: form }) : updateMutation.mutate({ id: editItem.id, data: form }); }} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>負責人</Label><Input value={form.assignedTo} onChange={e => setForm(f => ({ ...f, assignedTo: e.target.value }))} /></div>
                <div className="space-y-1.5">
                  <Label>狀態</Label>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={open => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader><AlertDialogTitle>確認刪除</AlertDialogTitle><AlertDialogDescription>確定要刪除這筆派工單嗎？</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter>
